#!/bin/sh
## based on https://raw.githubusercontent.com/anchore/grype/main/install.sh
...
# search_for_asset [checksums-file-path] [name] [os] [arch]
#
...
search_for_asset() (
  ...
  log_trace "search_for_asset(checksum-path=${checksum_path}, name=${name}, os=${os}, arch=${arch})"

  asset_glob="${name}_.*_${os}_${arch}"
...
download_and_install_asset() (
  ...
  asset_filepath=$(download_asset "${download_url}" "${download_path}" "${name}" "${os}" "${arch}" "${version}" "${binary}")
...
download_asset() (
  ...
  log_trace "download_asset(url=${download_url}, destination=${destination}, name=${name}, os=${os}, arch=${arch}, version=${version}, binary=${binary})"
  ...
  asset_filename=$(search_for_asset "${checksums_filepath}" "${binary}" "${os}" "${arch}")
...
#   (cd "${archive_dir}" && unpack "${asset_filepath}")
  ...
  install "${asset_filepath}" "${destination}/${binary}"
)
...
main() (
  ...
  for binary in ${BINARY_LIST}; do
    ...
    download_and_install_asset "${download_url}" "${download_dir}" "${install_dir}" "${PROJECT_NAME}" "${os}" "${arch}" "${version}" "${binary}"
